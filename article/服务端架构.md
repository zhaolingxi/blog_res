...
![total_frame](https://raw.githubusercontent.com/zhaolingxi/blog_res/refs/heads/master/image/20260301/total_frame.png)
## 2.业务模块整体架构设计
服务总体可以简化设计为：
![self_service](https://raw.githubusercontent.com/zhaolingxi/blog_res/refs/heads/master/image/20260301/self_service.png)
...
![use_case]([use_case](https://raw.githubusercontent.com/zhaolingxi/blog_res/refs/heads/master/image/20260301/use_case.png))
...
```
- name: Deploy from GitHub Release asset
  hosts: app_servers
  become: true
  vars:
    github_owner: "zhaolingxi"
    github_repo: "cicd_test"
    tag: "v1.2"                  # 运行时建议用 -e tag=... 覆盖
    asset_name: "hello-demo-0.1.0-Linux.tar.gz"

    install_root: "/opt/your_app"
    releases_dir: "{{ install_root }}/releases"
    current_link: "{{ install_root }}/current"
#    service_name: "your_app"       # systemd 服务名：your_app.service
  
    download_url: "https://github.com/{{ github_owner }}/{{ github_repo }}/releases/download/{{ tag }}/{{ asset_name }}"
    download_dest: "/tmp/{{ asset_name }}"
  
    # 私有仓库需要 token；公开仓库可不填
    github_token: ""               # 建议用 Ansible Vault 或环境变量注入
...
            | ternary({'Authorization': 'token ' ~ github_token}, {})
          }}
  
    - name: Create release dir for this tag
      ansible.builtin.file:
        path: "{{ releases_dir }}/{{ tag }}"
        state: directory
        mode: "0755"
 
    - name: Unarchive into release dir
      ansible.builtin.unarchive:
        src: "{{ download_dest }}"
        dest: "{{ releases_dir }}/{{ tag }}"
        remote_src: true
  
    - name: Update current symlink to this release
      ansible.builtin.file:
        src: "{{ releases_dir }}/{{ tag }}"
        dest: "{{ current_link }}"
        state: link
        force: true
